...
final class CaptureService: NSObject, SCStreamOutput, SCStreamDelegate, @unchecked Sendable {
    ...
    func start() async throws {
        ...
        let title=candidate.title ?? "Untitled FX";let stream=SCStream(filter:filter,configuration:config,delegate:self);try stream.addStreamOutput(self,type:SCStreamOutputType.screen,sampleHandlerQueue:queue);self.stream=stream;try await stream.startCapture();print("Capturing \(title) [\(config.width)x\(config.height)]")
    }
