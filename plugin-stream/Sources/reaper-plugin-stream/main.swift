...
final class CaptureService: NSObject, SCStreamOutput, SCStreamDelegate, @unchecked Sendable {
    ...
    func start() async throws {
        ...
        let stream=SCStream(filter:filter,configuration:config,delegate:self);try stream.addStreamOutput(self,type:.screen,sampleHandlerQueue:queue);self.stream=stream;try await stream.startCapture();print("Capturing \(candidate.title) [\(config.width)x\(config.height)]")
    }
