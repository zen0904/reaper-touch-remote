...
function openPlugin(track,fx){activeFX={trackId:track.id,fxIndex:fx.index,name:fx.name,enabled:fx.enabled};command("open_fx",`${track.id}|${fx.index}`);$("#pluginTrack").textContent=track.name;$("#pluginName").textContent=fx.name;renderRackChain(track);els.panel.dataset.key="";els.panel.innerHTML='<div class="plugin-message">Loading real parameters from REAPER…</div>';els.pluginView.hidden=false}
